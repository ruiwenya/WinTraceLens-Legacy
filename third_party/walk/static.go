...
func (s *static) init(widget Widget, parent Container, style uint32) error {
	...
	if err := s.group.toolTip.AddTool(s); err != nil {
		// Some Windows 7 / Server 2012 environments reject tooltip
		// registration for static controls. Tooltips are optional here, so
		// keep the control usable instead of failing window startup.
		return nil
	}
