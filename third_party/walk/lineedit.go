...
func (le *LineEdit) SetCueBanner(value string) error {
	if win.FALSE == le.SendMessage(win.EM_SETCUEBANNER, win.FALSE, uintptr(unsafe.Pointer(syscall.StringToUTF16Ptr(value)))) {
		// Cue banners are only placeholder text. Some systems return FALSE
		// here even though the edit control itself is usable, so keep startup
		// resilient and ignore this optional UI feature in the legacy build.
		return nil
	}
