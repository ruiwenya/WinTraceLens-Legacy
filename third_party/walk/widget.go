...
func (wb *WidgetBase) init(widget Widget) error {
	...
	if err := tt.AddTool(wb.window.(Widget)); err != nil {
		// Some legacy Windows environments reject TTM_ADDTOOL during control
		// creation. Tooltips are optional for WinTraceLens Legacy, so do not
		// fail the whole window when only tooltip registration fails.
		return nil
	}
