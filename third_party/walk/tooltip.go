...
func (tt *ToolTip) addTool(hwnd win.HWND, track bool) error {
	...
	if win.FALSE == tt.SendMessage(win.TTM_ADDTOOL, 0, uintptr(unsafe.Pointer(&ti))) {
		// Tooltips are optional. On some Windows 7 / Server 2012 systems the
		// common-controls tooltip window rejects TTM_ADDTOOL during startup,
		// but the actual control still works. Ignore this failure for the
		// legacy build so the UI can continue to load.
		return nil
	}
